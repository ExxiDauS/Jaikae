...
class AuthPermissionMiddleware:
    ...
    def __init__(self, get_response):
        ...
        def __call__(self, request):
            ...
            # Allow any path starting with /metrics
            allowed_prefixes = ["/metrics"]

            # If user isn't authenticated and trying to access protected pages
            if not request.user.is_authenticated:
                path_allowed = any(request.path == path for path in allowed_paths)
                prefix_allowed = any(request.path.startswith(p) for p in allowed_prefixes)
                if not (path_allowed or prefix_allowed):
